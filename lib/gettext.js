...
export default class Gettext {
    ...
    /**
     * Cache to avoid recreating translator functions.
     * @type Map<string, ReturnType<typeof this._bindLocale>>
     */
    #localeCache = new Map();
    /**
     ...
     */
    bindLocale(locales, domain = "messages") {
        const key = `${domain}|${locales}`;
        const cache = this.#localeCache;
        let current = cache.get(key);
        if (current) {
            return current;
        } else {
            current = this._bindLocale(locales, domain);
    ...
    // if we generate the key from the arguments with something like
    // `${domain}|${locales}`, anything more complex would literally just be
    // slower. At best it's a 2x speed increase, but we're talking about going
    // from 0.0002ms per bindLocale call to 0.0001ms here. It's not worth it.
    /**
     * Return functions that translate strings into `locale` within `domain`.
     * This allows not having global state but also not having to pass the
     * locale and domain for every call.
     ...
     * @param {Domain} [domain] - the domain, defaults to "messages"
     */
    _bindLocale(locales, domain = "messages") {
        let locale =
        ...
        if (typeof locales === "string" && locales.trim() === "") {
            this.warn("You specified an empty `locale` value.");
        }
        if (domain.trim() === "") {
         ...
         * @overload
         * @param {string} domain
         * @param {string | null | undefined} msgctxt
         * @param {string} msgid
